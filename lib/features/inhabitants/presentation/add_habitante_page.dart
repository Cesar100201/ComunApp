...
class _AddHabitantePageState extends State<AddHabitantePage> {
  ...
  void initState() {
    ...
    if (widget.initialCedula != null && widget.habitanteParaEditar == null) {
      _cedulaNumeroController.text = widget.initialCedula.toString();
    }
  ...
  Future<void> _cargarDatosParaEditar() async {
    ...
    if (habitanteCompleto != null) {
      ...
      // nivelUsuario solo lo edita el admin en caracterización; aquí se conserva el existente
      ...
      if (direccion.isNotEmpty) {
        ...
        if (_selectedConsejoComunal != null &&
            _selectedConsejoComunal!.comunidades.isNotEmpty) {
  ...
  // nivelUsuario: nuevos = invitado (3); en edición se conserva el existente (solo admin lo cambia en caracterización)
  ...
  Future<void> _filtrarLocalizaciones() async {
    ...
    _comunasFiltradas = _comunas
        .where((c) => c.parroquia == _selectedParroquia)
        .toList();
    ...
    if (_selectedComuna != null &&
        !_comunasFiltradas.contains(_selectedComuna)) {
    ...
    if (_selectedComuna != null) {
      ...
      _consejosComunalesFiltrados.sort(
        (a, b) => a.nombreConsejo.compareTo(b.nombreConsejo),
      );
  ...
  int _calcularEdad() {
    ...
    if (ahora.month < _selectedFechaNacimiento.month ||
        (ahora.month == _selectedFechaNacimiento.month &&
            ahora.day < _selectedFechaNacimiento.day)) {
      edad--;
    }
    return edad;
  }
    ...
    try {
      ...
      if (local != null) {
        ...
        if (local.jefeDeFamilia.value != null) {
          setState(() {
            _jefeEncontrado = null;
            _errorBusquedaJefe =
                "Error: Esta persona figura como carga familiar, no puede ser Jefe";
      ...
      try {
        ...
        if (remoto != null) {
          ...
          final isar = await DbHelper().db;
          final jefeLocal = await isar.habitantes
              .filter()
              .cedulaEqualTo(cedulaInt)
              .findFirst();
          if (jefeLocal == null) {
            ...
            setState(() {
              _jefeEncontrado = null;
              _errorBusquedaJefe =
                  "Jefe encontrado en la nube pero no en local. Por favor, sincronice los datos primero.";
              _isBuscandoJefe = false;
            });
            return;
          }
        ...
        setState(() {
          _jefeEncontrado = null;
          _errorBusquedaJefe =
              "Sin conexión: No se puede verificar el jefe en la nube";
          _isBuscandoJefe = false;
        });
        return;
      }
  ...
  void _parsearDireccion(String direccion) {
    ...
    for (var i = 0; i < partes.length; i++) {
      ...
      // Primer elemento puede ser estado
          ...
          !parte.toLowerCase().contains('calle') &&
          !parte.toLowerCase().contains('casa')) {
  ...
  String _construirDireccion() {
    ...
    if (_estadoController.text.trim().isNotEmpty) {
      partes.add(_estadoController.text.trim());
    }
    if (_municipioController.text.trim().isNotEmpty) {
      partes.add(_municipioController.text.trim());
    }
    partes.add(_selectedParroquia.toString().split('.').last);
    if (_selectedComuna != null) {
      partes.add(_selectedComuna!.nombreComuna);
    }
    if (_selectedConsejoComunal != null) {
      partes.add(_selectedConsejoComunal!.nombreConsejo);
    }
    if (_selectedComunidad.isNotEmpty) {
      partes.add(_selectedComunidad);
    }
    if (_calleController.text.trim().isNotEmpty) {
      partes.add("Calle ${_calleController.text.trim()}");
    }
    if (_numeroCasaController.text.trim().isNotEmpty) {
      partes.add("Casa ${_numeroCasaController.text.trim()}");
    }
  ...
  Future<void> _procesarGuardado({Habitante? habitanteParaActualizar}) async {
    ...
    try {
      ...
      // Nivel de usuario: nuevo habitante = invitado (3); edición = se conserva el actual (solo admin lo edita en caracterización)
      h.nivelUsuario =
          habitanteParaActualizar?.nivelUsuario ?? AppConstants.nivelInvitado;
      h.fotoUrl = null;
      h.isSynced = false;
      ...
      if (_esJefeDeFamilia) {
        ...
        h.jefeDeFamilia.value =
            null; // El modelo asume que si está vacío, es su propio jefe
      } else {
        // Asignar el jefe encontrado
        if (_jefeEncontrado != null) {
          final isar = await DbHelper().db;
          final jefeCompleto = await isar.habitantes
              .filter()
              .cedulaEqualTo(_jefeEncontrado!.cedula)
              .findFirst();
          if (jefeCompleto != null) {
            ...
            throw Exception(
              "El jefe de familia debe estar registrado localmente. Por favor, sincronice los datos primero.",
            );
          }
        }
      }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 15),
            const Text(
              "¿Desea sobrescribir los datos anteriores con la información nueva?",
            ),
          ],
        ),
        actions: [
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Expanded(child: _buildGeneroSelector()),
          ],
        ),
        const SizedBox(height: 15),
            ...
            children: [
              ...
              if (_comunasFiltradas.isNotEmpty)
                _buildDropdown<Comuna?>(
                  ...
                  itemToString: (Comuna? value) =>
                      value?.nombreComuna ?? "Seleccione una comuna...",
                ),
              ...
              if (_selectedComuna != null) ...[
                if (_consejosComunalesFiltrados.isNotEmpty)
                  _buildDropdown<ConsejoComunal?>(
                    ...
                    itemToString: (ConsejoComunal? value) =>
                        value?.nombreConsejo ??
                ...
                if (_consejosComunalesFiltrados.isEmpty &&
                    _selectedComuna != null)
                  Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
              ...
              if (_selectedConsejoComunal != null &&
                ...
                _buildDropdown<String>(
                  ...
                  itemToString: (String value) =>
                      value.isEmpty ? "Seleccione una comunidad..." : value,
                ),
                const SizedBox(height: 16),
              ],
            ],
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<Nacionalidad>(
              ...
              items: const [
                DropdownMenuItem(value: Nacionalidad.V, child: Text("V")),
                DropdownMenuItem(value: Nacionalidad.E, child: Text("E")),
              ],
        ...
        children: [
          ...
          Expanded(
            ...
            child: ElevatedButton.icon(
              onPressed: (_isSaving || _isCheckingRemote)
                  ? null
