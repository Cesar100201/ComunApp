...
/// Servicio optimizado para carga masiva de consejos comunales desde Excel/CSV.
///
...
class BulkUploadConsejosService {
  ...
  static Future<BulkUploadResult> procesarExcelEnSegundoPlano(
    ...
    try {
      ...
      if (esCSV) {
        AppLogger.info('Iniciando parseo de CSV en isolate (consejos)...');
        datosParseados = await compute(_parsearCSVEnIsolate, filePath);
      } else {
        AppLogger.info('Iniciando parseo de Excel en isolate (consejos)...');
        datosParseados = await compute(_parsearExcelEnIsolate, filePath);
      }
      AppLogger.info('Total de filas parseadas (consejos): ${datosParseados.length}');
      ...
      final comunasPorCodigo = <String, Comuna>{};
      for (var comuna in comunas) {
        comunasPorCodigo[comuna.codigoSitur.toLowerCase()] = comuna;
      }
      ...
      final loteBuffer = <ConsejoComunal>[];
      ...
      for (var i = 0; i < datosParseados.length; i++) {
        ...
        try {
          ...
          final nombreConsejo = (_getValue(rowMap, ['nombre consejo', 'nombreconsejo', 'consejo']) ?? '').trim();
          if (nombreConsejo.isEmpty) {
            result.errorCount++;
            result.errors.add('Fila ${i + 2}: Nombre del consejo es requerido');
            continue;
          }

          final rif = (_getValue(rowMap, ['rif']) ?? '').trim();
          final tipoZona = _parseTipoZona(_getValue(rowMap, ['tipo zona', 'tipozona', 'zona']));
          ...
          final comunidadesValue = _getValue(rowMap, ['comunidades', 'comunidad']);
          final comunidades = _parseComunidades(comunidadesValue);
          ...
          final comunaRef = _getValue(rowMap, ['codigo comuna', 'codigo situr comuna', 'codigo_situr_comuna']);
          if (comunaRef != null && comunaRef.isNotEmpty) {
            final key = comunaRef.toLowerCase();
            final comuna = comunasPorCodigo[key];
            if (comuna != null) {
              consejo.comuna.value = comuna;
            }
          }

          loteBuffer.add(consejo);
          result.successCount++;
          ...
          AppLogger.error('Error en fila ${i + 2} (consejos)', e, stackTrace);
        }
      }
      ...
      AppLogger.error('ERROR CRÍTICO en procesarExcelEnSegundoPlano (consejos)', e, stackTrace);
  ...
  static List<Map<String, dynamic>> _parsearCSVEnIsolate(String filePath) {
    try {
      ...
      if (kDebugMode) {
        debugPrint('Headers encontrados en CSV (consejos): ${headers.join(", ")}');
      }
  ...
  static List<Map<String, dynamic>> _parsearExcelEnIsolate(String filePath) {
    try {
      ...
      if (kDebugMode) {
        debugPrint('Headers encontrados en Excel (consejos): ${headers.join(", ")}');
      }
  ...
  static Future<void> _guardarLoteOptimizado(Isar isar, List<ConsejoComunal> lote) async {
    if (lote.isEmpty) return;

    try {
      final loteUnico = <String, ConsejoComunal>{};
      for (var consejo in lote) {
        loteUnico[consejo.codigoSitur] = consejo;
      }
      final loteSinDuplicados = loteUnico.values.toList();

      final codigos = loteSinDuplicados.map((c) => c.codigoSitur).toList();
      final existentesList = await isar.consejoComunals.getAllByCodigoSitur(codigos);

      final existentesMap = <String, ConsejoComunal>{};
      ...
      final nuevos = <ConsejoComunal>[];
      final actualizados = <ConsejoComunal>[];
      for (var consejo in loteSinDuplicados) {
        final existente = existentesMap[consejo.codigoSitur];
        if (existente != null) {
          existente.rif = consejo.rif;
          existente.nombreConsejo = consejo.nombreConsejo;
          existente.tipoZona = consejo.tipoZona;
          existente.latitud = consejo.latitud;
          existente.longitud = consejo.longitud;
          existente.comunidades = consejo.comunidades;
          if (consejo.comuna.value != null) {
            existente.comuna.value = consejo.comuna.value;
          }
          existente.isSynced = false;
          existente.isDeleted = false;
          actualizados.add(existente);
        } else {
          consejo.isDeleted = false;
          consejo.isSynced = false;
          nuevos.add(consejo);
        }
      }

      await isar.writeTxn(() async {
        final todosParaGuardar = <ConsejoComunal>[...nuevos, ...actualizados];
        if (todosParaGuardar.isNotEmpty) {
          await isar.consejoComunals.putAll(todosParaGuardar);
        }
        for (var consejo in todosParaGuardar) {
          if (consejo.comuna.value != null) {
      ...
      AppLogger.error('Error al guardar lote de ${lote.length} consejos', e);
      for (var consejo in lote) {
        try {
          await isar.writeTxn(() async {
            final existente = await isar.consejoComunals.getByCodigoSitur(consejo.codigoSitur);
            if (existente != null) {
              existente.rif = consejo.rif;
              existente.nombreConsejo = consejo.nombreConsejo;
              existente.tipoZona = consejo.tipoZona;
              ...
              existente.isSynced = false;
              existente.isDeleted = false;
              await isar.consejoComunals.put(existente);
              if (existente.comuna.value != null) {
                await existente.comuna.save();
              }
            } else {
              consejo.isDeleted = false;
              consejo.isSynced = false;
              await isar.consejoComunals.put(consejo);
              if (consejo.comuna.value != null) {
                await consejo.comuna.save();
              }
            }
          ...
          AppLogger.warning('Error al guardar consejo ${consejo.codigoSitur}: $e2');
        }
      }
    }
  }
  ...
  static TipoZona _parseTipoZona(String? value) {
    if (value == null || value.trim().isEmpty) return TipoZona.Urbano;
    final normalized = value.trim().toUpperCase();
    if (normalized.contains('RURAL')) return TipoZona.Rural;
    if (normalized.contains('MIX') || normalized.contains('MIXTO')) return TipoZona.Mixto;
    return TipoZona.Urbano;
  }

  static List<String> _parseComunidades(String? value) {
